...
let box_def =
  (Pair (vF, (vF::[])))::((Pair (vf, (vF::[])))::((Pair (vt, (vF::[])))::((Pair (vT,
    (vT::[])))::[])))

(** val dia_def : (nat, nat list) pair list **)

let dia_def =
  (Pair (vF, (vF::[])))::((Pair (vf, (vT::[])))::((Pair (vt, (vT::[])))::((Pair (vT,
    (vT::[])))::[])))
...
let makeRestrictionSteps a =
  restrictionSteps a (makeMatrix a) eqb_lf leb_lf geb_lf split_lf arrowsKT45 (S (S (S (S
    (S (S (S (S O)))))))) truleKT45
...
let makeThisRn _ a smallest lazymode =
  rnkripke a (makeMatrix a) eqb_lf leb_lf geb_lf length_lf split_lf arrowsKT45 (S (S (S
    (S (S (S (S (S O)))))))) d0 (Reflexive::(Symmetry::[])) smallest lazymode truleKT45
...
let makeCheckAllModels a smallest lazymode =
  checkAllModels a (makeMatrix a) eqb_lf leb_lf geb_lf split_lf arrowsKT45 val0 (S (S (S
    (S (S (S (S (S O)))))))) (makeAllRn a smallest lazymode) d0 truleKT45
