...
let box_def =
  (Pair (vF, (vF::[])))::((Pair (vf, (vF::[])))::((Pair (vf2, (vt2::[])))::((Pair (vt2,
    (vf2::(vf::(vF::[])))))::((Pair (vt, (vf2::(vf::(vF::[])))))::((Pair (vT,
    (vt2::(vt::(vT::[])))))::[])))))
...
let dia_def =
  (Pair (vF, (vf2::(vf::(vF::[])))))::((Pair (vf, (vt2::(vt::(vT::[])))))::((Pair (vf2,
    (vt2::(vt::(vT::[])))))::((Pair (vt2, (vf2::[])))::((Pair (vt, (vT::[])))::((Pair
    (vT, (vT::[])))::[])))))
...
let makeRestrictionSteps a =
  restrictionSteps a (makeMatrix a) eqb_lf leb_lf geb_lf split_lf arrowsKDB (S (S (S (S
    (S (S (S (S O)))))))) truleKDB
...
let makeThisRn _ a smallest lazymode =
  rnkripke a (makeMatrix a) eqb_lf leb_lf geb_lf length_lf split_lf arrowsKDB (S (S (S
    (S (S (S (S (S O)))))))) d0 (Symmetry::[]) smallest lazymode truleKDB
...
let makeCheckAllModels a smallest lazymode =
  checkAllModels a (makeMatrix a) eqb_lf leb_lf geb_lf split_lf arrowsKDB val0 (S (S (S
    (S (S (S (S (S O)))))))) (makeAllRn a smallest lazymode) d0 truleKDB
