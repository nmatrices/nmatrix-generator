...
let box_def =
  (Pair (vF, (vF::[])))::((Pair (vf, (vf2::(vf::(vF::[])))))::((Pair (vf2,
    (vT::[])))::((Pair (vt2, (vF::[])))::((Pair (vt, (vf2::(vf::(vF::[])))))::((Pair
    (vT, (vT::[])))::[])))))

(** val dia_def : (nat, nat list) pair list **)

let dia_def =
  (Pair (vF, (vF::[])))::((Pair (vf, (vt2::(vt::(vT::[])))))::((Pair (vf2,
    (vT::[])))::((Pair (vt2, (vF::[])))::((Pair (vt, (vt2::(vt::(vT::[])))))::((Pair
    (vT, (vT::[])))::[])))))
...
let makeRestrictionSteps a =
  restrictionSteps a (makeMatrix a) eqb_lf leb_lf geb_lf split_lf arrowsKD4 (S (S (S (S
    (S (S (S (S O)))))))) truleKD4
...
let makeThisRn _ a smallest lazymode =
  rnkripke a (makeMatrix a) eqb_lf leb_lf geb_lf length_lf split_lf arrowsKD4 (S (S (S
    (S (S (S (S (S O)))))))) d0 (Transitive::[]) smallest lazymode truleKD4
...
let makeCheckAllModels a smallest lazymode =
  checkAllModels a (makeMatrix a) eqb_lf leb_lf geb_lf split_lf arrowsKD4 val0 (S (S (S
    (S (S (S (S (S O)))))))) (makeAllRn a smallest lazymode) d0 truleKD4
