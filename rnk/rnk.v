...
Fixpoint validate_aux
  ...
  | f::tl =>
      ...
      | None =>
          true
      | Some (B, targets) =>
          let wB := applyTo w B 404 eqb_X in
          if
            andb
              (negb (Nat.eqb wB 404))
...
Compute power [1;2;3] 404.

Compute power [1; 4; 5; 6] 0.
...
Fixpoint backtracking_aux1
  ...
  | bun (A, v) nt =>
      let fA := applyTo filtro A 404 eqb_X in
      if andb (negb (Nat.eqb fA 404)) (negb (Nat.eqb v fA)) then
        bleaf nil false
      else
...
Definition backtracking
  ...
  if isElementInList A (map (fun x => proj_l x) filtro) eqb_X then
    let firstf := (A, applyTo filtro A 404 eqb_X) in
...
Fixpoint expandsemtree
  ...
  | smleaf toExpand pv =>
      if negb (isEmpty toExpand) then
        ...
        let vA := applyTo pv A 404 eqb_X in
